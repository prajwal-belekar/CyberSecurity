...
import { extractFeatures, parseTargetUrl, registrationFor, scoreFromFeatures, severityFromScore, verdictFromScore, } from '@/utils/urlAnalysis';
export const phishingApi = {
    async history() {
        if (!USE_MOCK)
            return apiRequest(ENDPOINTS.phishingHistory);
        await guardFailures(ENDPOINTS.phishingHistory);
        await simulateLatency();
        return [...mockStore.phishingScans].sort((a, b) => +new Date(b.scannedAt) - +new Date(a.scannedAt));
    },
    /**

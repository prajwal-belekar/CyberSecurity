...
export const threatsApi = {
    async list(filters = {}) {
        if (!USE_MOCK)
            return apiRequest(ENDPOINTS.threats, { query: filters });
        await guardFailures(ENDPOINTS.threats);
        await simulateLatency();
        const search = filters.search?.trim().toLowerCase();
        return mockStore.threats.filter((threat) => {
            if (filters.severity?.length && !filters.severity.includes(threat.severity))
                return false;
            if (filters.status?.length && !filters.status.includes(threat.status))
                return false;
            if (filters.type?.length && !filters.type.includes(threat.type))
                return false;
            if (filters.source && !threat.source.toLowerCase().includes(filters.source.toLowerCase()))
                return false;
            if (filters.timeRange && filters.timeRange !== 'all' && !isWithinRange(threat.lastSeen, filters.timeRange))
                return false;
            if (search) {
    ...
    async byId(id) {
        if (!USE_MOCK)
            return apiRequest(`${ENDPOINTS.threats}/${id}`);
        await simulateLatency(80, 220);
        return mockStore.threats.find((t) => t.id === id);
    },
    async updateStatus(id, status) {
        if (!USE_MOCK)
            return apiRequest(`${ENDPOINTS.threats}/${id}`, { method: 'PATCH', body: { status } });
        await simulateLatency(120, 320);
        return mockStore.updateThreat(id, { status });
    },
    async activity(range = '24H') {
        if (!USE_MOCK)
            return apiRequest(`${ENDPOINTS.threats}/activity`, { query: { range } });

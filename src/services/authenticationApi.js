...
export const authenticationApi = {
    async summary() {
        if (!USE_MOCK)
            return apiRequest(ENDPOINTS.authSummary);
    ...
    async events(query = {}) {
        if (!USE_MOCK)
            return apiRequest(ENDPOINTS.authEvents, { query: query });
        ...
        const filtered = AUTHENTICATION_EVENTS.filter((event) => {
            if (query.status && query.status !== 'all' && event.status !== query.status)
                return false;
            if (query.risk && query.risk !== 'all' && event.risk !== query.risk)
                return false;
    ...
    async sequences() {
        if (!USE_MOCK)
            return apiRequest(ENDPOINTS.authSequences);
        await simulateLatency(140, 340);
        return AUTH_SEQUENCES;
    },
    /** Hourly successful vs failed logins for the authentication chart. */
    async outcomesSeries(range = '24H') {
        if (!USE_MOCK)
            return apiRequest(`${ENDPOINTS.authEvents}/series`, { query: { range } });

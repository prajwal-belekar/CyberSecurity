...
import type { AiSession, ChatMessage, InvestigationContext, SuggestedPrompt } from '@/types/ai';
...
function contextIncident(ctx: InvestigationContext): Incident | undefined {
  return ctx.incidentId ? mockStore.incidents.find((i) => i.id === ctx.incidentId) : undefined;
}

function contextThreat(ctx: InvestigationContext): Threat | undefined {
  if (ctx.threatId) return mockStore.threats.find((t) => t.id === ctx.threatId);
  const incident = contextIncident(ctx);
  return incident ? mockStore.threats.find((t) => t.incidentId === incident.id) : undefined;
}

function contextEvents(ctx: InvestigationContext) {
  const incident = contextIncident(ctx);
  ...
  const explicit = ids.map((id) => byId.get(id)).filter((e): e is NonNullable<typeof e> => Boolean(e));
  const correlated = incident
    ? mockStore.events.filter((e) => e.incidentId === incident.id)
    : [];
  const merged = [...explicit, ...correlated].filter(
...
function answerFor(question: string, ctx: InvestigationContext): Answer {
  const q = question.toLowerCase().trim();
  const incident = contextIncident(ctx);
  const threat = contextThreat(ctx);
  const events = contextEvents(ctx);
  const citations: ChatMessage['citations'] = [
    ...(incident ? [{ id: incident.id, label: incident.id, type: 'incident' as const, href: `/incidents/${incident.id}` }] : []),
    ...(threat ? [{ id: threat.id, label: threat.id, type: 'event' as const, href: `/threats/${threat.id}` }] : []),
    ...events.slice(0, 4).map((e) => ({ id: e.id, label: e.id, type: 'event' as const })),
  ];

  if (!incident && !threat && !events.length) {
    return {
      content:
        'No investigation context is attached yet. Select an incident from the Evidence panel — or use the command palette (Ctrl+K) and run `investigate INC-2048` — and I will ground every answer in that record.',
      confidence: 1,
    };
  }
  ...
  if (/why.*detect|detection rule|what triggered|why was this/.test(q)) {
    const rules = Array.from(new Set(events.map((e) => e.detectionRule).filter(Boolean))) as string[];
    return {
      content: [
  ...
  return {
    content:
      `Working from ${incident?.id ?? threat?.id ?? 'the attached evidence'}: ${events.length} correlated events from ${source} against ${target}, severity ${severityMeta(severity).label}, last activity ${events.length ? formatRelative(events[events.length - 1]!.timestamp) : 'unavailable'}.\n\n` +
...
export const aiApi = {
  ...
  async investigate(question: string, ctx: InvestigationContext): Promise<ChatMessage> {
    if (!USE_MOCK) {
      return apiRequest<ChatMessage>(ENDPOINTS.aiInvestigate, { method: 'POST', body: { question, context: ctx } });
    }
  ...
  createSession(ctx: InvestigationContext, title = 'Investigation'): AiSession {
    return {

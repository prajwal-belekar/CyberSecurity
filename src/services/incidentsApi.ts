import type { Incident, IncidentNote, IncidentStatus, IncidentSummary, IncidentTimelineEntry } from '@/types/incident';
...
function recomputeSummary(): IncidentSummary {
  const counts = mockStore.incidents.reduce(
...
export const incidentsApi = {
  async list(status?: IncidentStatus | 'all'): Promise<Incident[]> {
    if (!USE_MOCK) return apiRequest(ENDPOINTS.incidents, { query: { status } });
    await guardFailures(ENDPOINTS.incidents);
    await simulateLatency();
    const all = [...mockStore.incidents].sort(
      (a, b) => +new Date(b.createdAt) - +new Date(a.createdAt),
    );
    return !status || status === 'all' ? all : all.filter((i) => i.status === status);
  },

  async byId(id: string): Promise<Incident | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/${id}`);
    await simulateLatency(100, 260);
    return mockStore.incidents.find((i) => i.id === id);
  },

  async evidence(id: string): Promise<SecurityEvent[]> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/${id}/evidence`);
    ...
    const byIncident = mockStore.events.filter(
      (e) => e.incidentId === id && !incident.evidenceEventIds.includes(e.id),
    );
    return [...linked, ...byIncident].sort(
      (a, b) => +new Date(a.timestamp) - +new Date(b.timestamp),
    );
  },

  async summary(): Promise<IncidentSummary> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/summary`);
    await simulateLatency(70, 180);
    return recomputeSummary();
  },

  async updateStatus(id: string, status: IncidentStatus, actor = 'a.reyes'): Promise<Incident | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/${id}`, { method: 'PATCH', body: { status } });
  ...
  async assign(id: string, assignedTo: string, actor = 'a.reyes'): Promise<Incident | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/${id}/assign`, { method: 'POST', body: { assignedTo } });
  ...
  async addNote(id: string, body: string, author = 'a.reyes'): Promise<Incident | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/${id}/notes`, { method: 'POST', body: { body } });
    await simulateLatency(160, 380);
    const incident = mockStore.incidents.find((i) => i.id === id);
    if (!incident) return undefined;
    const note: IncidentNote = { id: `note-${Date.now()}`, author, timestamp: new Date().toISOString(), body };
    const entry: IncidentTimelineEntry = {
      id: `tl-${Date.now()}`,
      time: formatClockShort(new Date()),
      timestamp: new Date().toISOString(),
  ...
  async escalateToIncident(eventId: string): Promise<{ incidentId: string } | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.incidents}/from-event`, { method: 'POST', body: { eventId } });
    await simulateLatency(200, 460);
    const event = mockStore.events.find((e) => e.id === eventId);
    if (!event) return undefined;
    if (event.incidentId) return { incidentId: event.incidentId };

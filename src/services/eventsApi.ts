import type { SecurityEvent, ThreatSummary } from '@/types/threat';
...
export interface EventQuery extends ApiListParams {
  severity?: string;
  status?: string;
  timeRange?: string;
  channel?: string;
...
function filterEvents(query: EventQuery): SecurityEvent[] {
  ...
  return mockStore.events.filter((event) => {
    ...
    if (query.channel && query.channel !== 'all' && event.channel !== query.channel) return false;
...
export const eventsApi = {
  async list(query: EventQuery = {}): Promise<{ items: SecurityEvent[]; total: number }> {
    if (!USE_MOCK) {
      return apiRequest(ENDPOINTS.events, { query: query as Record<string, string> });
    }
  ...
  async recent(limit = 8): Promise<SecurityEvent[]> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.events}?limit=${limit}`);
    await guardFailures(ENDPOINTS.events);
    await simulateLatency(90, 240);
    return mockStore.events.slice(0, limit);
  },

  async byId(id: string): Promise<SecurityEvent | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.events}/${id}`);
    await simulateLatency(60, 160);
    return mockStore.events.find((e) => e.id === id);
  },

  async byIncident(incidentId: string): Promise<SecurityEvent[]> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.events}?incidentId=${incidentId}`);
  ...
  async related(event: SecurityEvent, limit = 8): Promise<SecurityEvent[]> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.events}/${event.id}/related`);
    await simulateLatency(80, 200);
  ...
  async summary(): Promise<ThreatSummary> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.events}/summary`);
    await guardFailures(ENDPOINTS.events);
    await simulateLatency(80, 200);
  ...
  async setStatus(id: string, status: SecurityEvent['status']): Promise<SecurityEvent | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.events}/${id}`, { method: 'PATCH', body: { status } });
    await simulateLatency(80, 200);

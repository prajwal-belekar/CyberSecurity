...
export const API_TIMEOUT_MS = Number(
  (import.meta.env.VITE_API_TIMEOUT as string | undefined) ?? 15_000,
);

export const API_MODE: 'mock' | 'live' =
  (import.meta.env.VITE_API_MODE as 'mock' | 'live' | undefined) ?? 'mock';
...
export class ApiError extends Error {
  ...
  constructor(message: string, status = 503, endpoint = '', hint?: string) {
    super(message);
    this.name = 'ApiError';
    this.status = status;
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(
      ...
      503,
      path,
      'Confirm the FastAPI backend is running and VITE_API_BASE_URL is correct.',
    );
  } finally {
    clearTimeout(timer);
  }

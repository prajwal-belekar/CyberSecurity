...
export const reportsApi = {
  ...
  async list(type?: ReportType | 'all'): Promise<Report[]> {
    if (!USE_MOCK) return apiRequest(ENDPOINTS.reports, { query: { type } });
  ...
  async byId(id: string): Promise<Report | undefined> {
    if (!USE_MOCK) return apiRequest(`${ENDPOINTS.reports}/${id}`);
   ...
   */
  async generate(request: ReportGenerationRequest): Promise<Report> {
    if (!USE_MOCK) return apiRequest(ENDPOINTS.reportsGenerate, { method: 'POST', body: request });
   ...
   */
  buildDownload(report: Report): { filename: string; blob: Blob } {
    const payload = {

 ...
 */
export function validateScanAuthorization(rawTarget, authorization) {
...
export const webSecurityApi = {
    async previousScan() {
        if (!USE_MOCK)
            return apiRequest(`${ENDPOINTS.webSecurityScan}/latest`);
     ...
     */
    async completeScan(target, token) {
        if (!USE_MOCK)
            return apiRequest(ENDPOINTS.webSecurityScan, { method: 'POST', body: { target, token } });

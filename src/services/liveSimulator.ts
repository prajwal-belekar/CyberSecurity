...
export type LiveListener = (event: ReturnType<typeof instantiateScriptEvent>) => void;
...
function nextTickMs(): number {
  ...
  if (!current) return 12_000;
  if (!next) return 12_000;
  const delta = Math.max(TICK_FLOOR_SECONDS, next.atSeconds - current.atSeconds);
  return delta * 1000 + Math.round(rng() * 1200);
}

function jitterMetrics(): void {
  const m: SystemMetrics = mockStore.metrics;
...
function fire(): void {
  ...
  if (step) {
    ...
    listeners.forEach((l) => { try { l(event); } catch { /* subscriber isolation */ } });

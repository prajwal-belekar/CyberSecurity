...
export type SearchCategory =
  | 'incidents' | 'threats' | 'events' | 'ip_addresses' | 'domains'
  | 'users' | 'indicators' | 'reports' | 'assets';
...
export interface SearchResult {
  ...
  href?: string;
  severity?: Severity;
  /** Monospace rendering for IPs, hashes and IDs. */
  mono?: boolean;
}
...
export const searchApi = {
  async search(rawQuery: string, limit = MAX_PER_CATEGORY): Promise<SearchResult[]> {
    const query = rawQuery.trim().toLowerCase();
    if (!query) return [];
    if (!USE_MOCK) return apiRequest<SearchResult[]>('/search', { query: { q: query } });
    ...
    [...new Set(mockStore.events.flatMap((e) => [e.source, e.target]).filter((v): v is string => Boolean(v)))]

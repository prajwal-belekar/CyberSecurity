 ...
 */
const ALLOWED = ['help', 'status', 'threats', 'network', 'incidents', 'intel', 'analytics', 'reports', 'settings', 'auth', 'phishing', 'scanner', 'malware', 'dashboard', 'clear', 'stream', 'investigate', 'analyze', 'metrics', 'whoami', 'version'];
export function TerminalDock() {
        ...
        switch (cmd) {
            case 'help':
                append([
                    ...
                    ].map((line, i) => ({ id: `h-${Date.now()}-${i + 2}`, kind: 'output', text: line, tone: 'text-ink-2' })),
                ]);
                break;
            case 'status': {
            ...
            case 'threats': {
                if (arg) {
                    ...
                    if (found) {
                        navigate(`/threats/${found.id}`);
                        append([{ id: `t-${Date.now()}`, kind: 'output', text: `Opening ${found.id} — ${found.title}`, tone: 'text-term' }]);
                        break;
                    }
            ...
            case 'incidents': {
                if (arg) {
                    ...
                    if (found) {
                        navigate(`/incidents/${found.id}`);
                        append([{ id: `i-${Date.now()}`, kind: 'output', text: `Opening ${found.id} — ${found.title}`, tone: 'text-term' }]);
                        break;
                    }
            ...
            case 'network':
                navigate('/network');
                append([{ id: `n-${Date.now()}`, kind: 'output', text: 'Opening network monitor…', tone: 'text-term' }]);
                break;
            case 'auth':
                navigate('/authentication');
                append([{ id: `a-${Date.now()}`, kind: 'output', text: 'Opening authentication monitor…', tone: 'text-term' }]);
                break;
            case 'intel':
                navigate('/threat-intelligence');
                append([{ id: `it-${Date.now()}`, kind: 'output', text: 'Opening threat intelligence…', tone: 'text-term' }]);
                break;
            case 'analytics':
                navigate('/analytics');
                append([{ id: `an-${Date.now()}`, kind: 'output', text: 'Opening analytics workspace…', tone: 'text-term' }]);
                break;
            case 'reports':
                navigate('/reports');
                append([{ id: `r-${Date.now()}`, kind: 'output', text: 'Opening report archive…', tone: 'text-term' }]);
                break;
            case 'settings':
                navigate('/settings');
                append([{ id: `se-${Date.now()}`, kind: 'output', text: 'Opening workspace settings…', tone: 'text-term' }]);
                break;
            case 'dashboard':
                navigate('/dashboard');
                append([{ id: `d-${Date.now()}`, kind: 'output', text: 'Opening command center…', tone: 'text-term' }]);
                break;
            case 'phishing':
            case 'analyze':
                navigate('/phishing');
                ...
                break;
            case 'scanner':
                navigate('/web-security');
                append([{ id: `sc-${Date.now()}`, kind: 'output', text: 'Opening web security scanner…', tone: 'text-term' }]);
                break;
            case 'malware':
                navigate('/malware');
                append([{ id: `ml-${Date.now()}`, kind: 'output', text: 'Opening malware analyzer…', tone: 'text-term' }]);
                break;
            case 'whoami':
    ...
    return (_jsx(AnimatePresence, { initial: false, children: terminalDockOpen ? (_jsxs(motion.div, { initial: { height: 0, opacity: 0 }, animate: { height: 'auto', opacity: 1 }, exit: { height: 0, opacity: 0 }, transition: { duration: 0.2, ease: [0.22, 1, 0.36, 1] }, className: "z-20 shrink-0 overflow-hidden border-t border-line bg-void", children: [_jsxs("div", { className: "flex h-7 items-center gap-1 border-b border-line bg-base px-2", children: [_jsx(TerminalIcon, { className: "size-3 text-term", "aria-hidden": true }), tabs.map((t) => (_jsx("button", { type: "button", onClick: () => setTab(t.id), "aria-pressed": tab === t.id, className: cn('mono rounded-[2px] px-2 py-0.5 text-[11px] font-semibold tracking-[0.02em] uppercase transition-colors', tab === t.id ? 'bg-term/12 text-term' : 'text-ink-4 hover:text-ink-2'), children: t.label }, t.id))), _jsx("span", { className: "flex-1" }), tab === 'terminal' ? (_jsx("button", { type: "button", onClick: () => setLines([]), "aria-label": "Clear console output", className: "inline-flex size-5 items-center justify-center rounded-[2px] text-ink-4 transition-colors hover:bg-raised hover:text-ink", children: _jsx(Trash2, { className: "size-3", "aria-hidden": true }) })) : null, _jsx("button", { type: "button", onClick: toggleTerminalDock, "aria-label": "Collapse terminal dock", className: "inline-flex size-5 items-center justify-center rounded-[2px] text-ink-4 transition-colors hover:bg-raised hover:text-ink", children: _jsx(ChevronDown, { className: "size-3.5", "aria-hidden": true }) })] }), _jsxs("div", { className: "h-[168px] sm:h-[184px]", children: [tab === 'terminal' ? (_jsxs("div", { className: "flex h-full flex-col", children: [_jsx("div", { ref: bodyRef, className: "min-h-0 flex-1 overflow-y-auto px-2.5 py-1.5", children: lines.map((line) => (_jsx("div", { className: cn('term-line', line.kind === 'input' && 'flex gap-2', line.tone ?? 'text-ink-2'), children: line.kind === 'input' ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "prompt shrink-0 select-none", children: "root@cybersentinel:~$" }), _jsx("span", { className: "text-ink", children: line.text })] })) : (line.text) }, line.id))) }), _jsxs("div", { className: "flex shrink-0 items-center gap-2 border-t border-line bg-base px-2.5 py-1.5", children: [_jsx(Prompt, { className: "text-[11px]" }), _jsx("input", { ref: inputRef, value: value, onChange: (e) => setValue(e.target.value), onKeyDown: onKeyDown, spellCheck: false, autoComplete: "off", "aria-label": "Terminal command input", placeholder: "type a command \u2014 try: status", className: "mono min-w-0 flex-1 bg-transparent text-[11.5px] text-ink caret-term placeholder:text-ink-4 focus:outline-none" }), _jsx(Caret, {})] })] })) : null, tab === 'stream' ? (_jsx(EventStream, { showHeader: false, limit: 40, maxHeight: "100%", className: "h-full", onSelect: openEvent })) : null, tab === 'log' ? (_jsx("div", { className: "h-full overflow-y-auto px-2.5 py-1.5", children: stream.slice(0, 26).map((event) => {
                                const meta = severityMeta(event.severity);
                                return (_jsxs("div", { className: "term-line flex flex-wrap items-baseline gap-x-2 border-b border-panel-2 py-[2px]", children: [_jsx("span", { className: "mono tnum shrink-0 text-[11px] text-ink-4", children: formatClock(event.timestamp) }), _jsx("span", { className: cn('mono shrink-0 text-[11px] font-bold', meta.text), children: meta.label }), _jsx("span", { className: "mono shrink-0 text-[11px] text-cyber", children: event.channel }), _jsx("span", { className: "min-w-0 flex-1 truncate text-[10.5px] text-ink-2", children: event.type }), _jsx("span", { className: "mono hidden shrink-0 text-[11px] text-ink-4 md:inline", children: event.source }), event.detectionRule ? (_jsx("span", { className: "mono hidden shrink-0 rounded-[2px] border border-line-2 px-1 text-[10.5px] text-ink-4 lg:inline", children: event.detectionRule })) : null] }, event.id));
                            }) })) : null] })] }, "dock")) : null }));
}

...
/** 17:42 */
export function formatClockShort(value) {
    const d = new Date(value);
    if (Number.isNaN(d.getTime()))
        return '--:--';
    const p = (n) => String(n).padStart(2, '0');
...
export function isWithinRange(timestamp, range, now = Date.now()) {
    const window = TIME_RANGE_MS[range];
    if (!window)
        return true;
    return now - new Date(timestamp).getTime() <= window;
}
export function minutesAgo(minutes, from = Date.now()) {
    return new Date(from - minutes * 60_000).toISOString();
}
export function hoursAgo(hours, from = Date.now()) {

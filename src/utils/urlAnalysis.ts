...
export function parseTargetUrl(raw: string): ParsedUrl {
...
export function extractFeatures(parsed: ParsedUrl): PhishingFeatures {
  ...
  const lookalikeBrand = BRAND_TERMS.find((brand) => {
    if (!combined.includes(brand)) return false;
  ...
  const suspiciousKeywords = Array.from(
    new Set(SUSPICIOUS_KEYWORDS.filter((k) => combined.includes(k))),
  ).slice(0, 5);
...
export function scoreFromFeatures(features: PhishingFeatures): number {
  let score = 0;
  if (features.domainAgeDays < 7) score += 22;
  else if (features.domainAgeDays < 30) score += 16;
  else if (features.domainAgeDays < 90) score += 6;
  if (features.lookalikeBrand) score += 24;
  if (features.subdomainDepth > 3) score += 12;
  else if (features.subdomainDepth > 2) score += 7;
  if (features.protocol === 'http:') score += 16;
  if (features.hasLoginForm) score += 14;
  if (features.isIpHost) score += 20;
  if (features.length > 90) score += 8;
  else if (features.length > 70) score += 4;
  if (features.redirects > 1) score += 10;
  if (features.certIssuer.startsWith('Let')) score += 6;
  score += features.reputationHits * 14;
  score += features.suspiciousKeywords.length * 5;
  return Math.max(1, Math.min(99, score));
}

export function verdictFromScore(score: number): 'safe' | 'suspicious' | 'phishing' | 'malicious' {
...
export function registrationFor(host: string) {

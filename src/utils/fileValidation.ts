...
const ALLOWED_MIME_PREFIXES: readonly string[] = ['application/', 'text/', 'multipart/'];
 ...
 */
export function isAllowedFileType(mimeOrName: string): boolean {
  ...
  if (value.includes('/')) return ALLOWED_MIME_PREFIXES.some((prefix) => value.startsWith(prefix));
  return false;
}
...
export function validateSubmission(fileName: string, sizeBytes: number, mime = ''): FileValidationResult {
  if (!fileName.trim()) return { ok: false, reason: 'No file selected.' };
  if (sizeBytes <= 0) return { ok: false, reason: 'The selected file is empty.' };

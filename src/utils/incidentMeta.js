...
export const INCIDENT_STATUS_ORDER = ['open', 'investigating', 'contained', 'resolved', 'false_positive'];
export const priorityMeta = (priority) => PRIORITY_META[priority];
export const statusMetaFor = (status) => STATUS_META[status];
/** SLA position: 'ok' | 'warning' | 'breached' for the elapsed clock. */
export function slaState(createdAt, priority, status, now = Date.now()) {
    ...
    if (STATUS_META[status].terminal)
        return { label: 'CLOCK STOPPED', tone: 'term', elapsedMinutes };
    if (elapsedMinutes > target)
        return { label: `SLA BREACHED · ${elapsedMinutes}m / ${target}m`, tone: 'critical', elapsedMinutes };
    if (elapsedMinutes > target * 0.7)
        return { label: `AT RISK · ${elapsedMinutes}m / ${target}m`, tone: 'medium', elapsedMinutes };

...
import type { EventChannel } from '@/types/threat';

/**
 * Deterministic live-event script.
 *
 * The spec explicitly requires that the simulated real-time feed NOT be random
 * noise: it must escalate through a believable chain
 *   normal → failed login → repeated failures → suspicious pattern → threat → incident
 * and then settle back into ambient operational traffic before repeating.
 *
 * The FastAPI backend will replace this with a WebSocket feed; the UI contract
 * (SecurityEvent[]) stays identical.
 */
...
export const SCENARIO_SCRIPT: ScriptedEvent[] = [
...
export function instantiateScriptEvent(step: ScriptedEvent, now = Date.now()): SecurityEvent {
  scriptCounter += 1;
  return {
